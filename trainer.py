...
class Trainer():
    def __init__(self, args, logger, dataloader, model, loss_all):
        ...
        self.optimizer = optim.Adam(self.params, betas=(args.beta1, args.beta2),
                                    eps=self.args.eps, lr=self.args.lr_rate)
        ...
        self.args.rec_w = 1.0
        self.args.per_w = 0.05
        self.args.tql_w = 0.05
        self.args.adv_w = 0.0001

    def load(self, model_path):
        self.logger.info('load_model_path: ' + model_path)
        model_state_dict = torch.load(model_path, map_location=self.device)
        model_dict = self.model.state_dict()
        pretrained_dict = {k: v for k, v in model_state_dict.items() if
        ...
        for i_batch, sample_batched in enumerate(self.dataloader['train']):
            ...
            if (i_batch % 100 == 0):
                save_path = os.path.join(self.args.save_dir,
                                         ...
                                         f'epoch_{current_epoch}_batch_{i_batch}.png')
                os.makedirs(os.path.dirname(save_path), exist_ok=True)
                torchvision.utils.save_image(sr.detach(), save_path, normalize=True)
            loss.backward()
            self.optimizer.step()
